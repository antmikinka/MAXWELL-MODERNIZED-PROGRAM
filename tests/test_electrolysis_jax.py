...
class TestFaradayLawsJAXPytree:
    ...
    def test_jit_compatible(self):
        obj = FaradayLawsJAX()
        jit_fn = jax.jit(lambda o: o.mass_from_charge(
...
class TestIonTransportJAXPytree:
    ...
    def test_jit_migration(self):
        obj = IonTransportJAX(
            ion_mobilities=jnp.array([5.19e-4]),
            ion_charges=jnp.array([1.0]))
    ...
    def test_tree_map(self):
        obj = IonTransportJAX(
            ion_mobilities=jnp.array([1.0, 2.0]),
...
class TestElectrolysisCellJAXComputation:
    ...
    def test_analyze_returns_dict(self):
        ...
        expected_keys = {
            "mass_deposited", "charge_passed", "cell_resistance",
            "ir_drop", "overpotential", "required_voltage",
            "energy_consumed", "energy_per_gram", "power"
        }
    ...
    def test_analyze_mass_consistency(self):
        ...
        assert float(result["mass_deposited"]) == pytest.approx(float(m_direct), rel=1e-10)
...
class TestStandaloneElectrolysisFunctions:
    ...
    def test_faraday_first_law(self):
        m = faraday_first_law_jax(
            jnp.array(1.0), jnp.array(100.0), jnp.array(0.001))
    ...
    def test_polarization_emf(self):
        E = polarization_emf_jax(
            jnp.array(1.23e8), jnp.array(0.01), jnp.array(1e-6),
            jnp.array(0.5), jnp.array(298.15))
        assert float(E) > 1.23e8  # Should be greater than reversible

    def test_decomposition_voltage(self):
        V = decomposition_voltage_jax(
            jnp.array(1.23e8), jnp.array(0.4e8),
            jnp.array(-0.1e8), jnp.array(0.05e8))
        expected = 1.23e8 + 0.4e8 + 0.1e8 + 0.05e8
    ...
    def test_kohlrausch_law(self):
        Lambda = kohlrausch_law_jax(
            jnp.array(149.9), jnp.array(0.01), jnp.array(94.0))
    ...
    def test_concentration_polarization(self):
        eta = concentration_polarization_jax(
            jnp.array(1e-4), jnp.array(1e-5),
            jnp.array(7e-6), jnp.array(0.01),
            jnp.array(0.01), jnp.array(2.0), jnp.array(298.15))
        assert float(eta) < 0  # Depletion -> negative overpotential

    def test_battery_back_emf(self):
        V = battery_back_emf_jax(
            jnp.array(1.1e8), jnp.array(1e9),
...
class TestAutoDiffElectrolysis:
    ...
    def test_grad_mass_wrt_current(self):
        grad_fn = jax.grad(lambda I: faraday_second_law_jax(
    ...
    def test_grad_mass_wrt_molar_mass(self):
        grad_fn = jax.grad(lambda M: faraday_second_law_jax(
    ...
    def test_grad_activation_wrt_current_density(self):
        grad_fn = jax.grad(lambda j: polarization_emf_jax(
            jnp.array(1.23e8), j, jnp.array(1e-6),
            jnp.array(0.5), jnp.array(298.15)))
        g = grad_fn(0.01)
    ...
    def test_grad_conductivity_wrt_concentration(self):
        grad_fn = jax.grad(lambda c: electrolyte_conductivity_jax(
            jnp.array([c, c]),
            jnp.array([1.0, -1.0]),
            jnp.array([5.19e-4, 7.91e-4])))
        g = grad_fn(1e-4)
    ...
    def test_grad_cell_resistance_wrt_spacing(self):
        grad_fn = jax.grad(lambda d: ElectrolysisCellJAX(
            electrode_area=1.0,
            electrode_spacing=d,
            electrolyte_conductivity=0.1,
            molar_mass=63.55,
            valence=2,
            reversible_emf=1.1e8).cell_resistance())
        g = grad_fn(1.0)
        expected = 1.0 / (0.1 * 1.0)
        assert float(g) == pytest.approx(expected, rel=1e-10)
...
class TestVmapElectrolysis:
    ...
    def test_vmap_faraday_second_law(self):
        ...
        masses = jax.vmap(lambda I: faraday_second_law_jax(
            I, jnp.array(100.0), jnp.array(63.55), jnp.array(2.0)))(currents)
    ...
    def test_vmap_migration_velocity(self):
        ...
        velocities = jax.vmap(lambda E: ion_migration_velocity_jax(
            jnp.array(5.19e-4), E, jnp.array(1.0)))(fields)
    ...
    def test_vmap_kohlrausch_law(self):
        ...
        lambdas = jax.vmap(lambda c: kohlrausch_law_jax(
            jnp.array(149.9), c, jnp.array(94.0)))(concs)
    ...
    def test_vmap_decomposition_voltage(self):
        ...
        voltages = jax.vmap(lambda ir: decomposition_voltage_jax(
            jnp.array(1.23e8), jnp.array(0.4e8),
            jnp.array(-0.1e8), ir))(ohmic_drops)
...
class TestNumPyElectrolysisComparison:
    ...
    def test_kohlrausch_law_numpy_equiv(self):
        ...
        jax_result = kohlrausch_law_jax(
            jnp.array(L0), jnp.array(c), jnp.array(K))
        assert float(jax_result) == pytest.approx(float(np_result))
    ...
    def test_decomposition_voltage_numpy_equiv(self):
        ...
        jax_result = decomposition_voltage_jax(
            jnp.array(E_rev), jnp.array(eta_a),
            jnp.array(eta_c), jnp.array(ir))
        assert float(jax_result) == pytest.approx(float(np_result))
...
class TestVerifyElectrolysis:
    ...
    def test_verify_keys_present(self):
        result = verify_electrolysis_jax()
        expected_keys = {
            "first_law_ok", "roundtrip_ok", "transference_ok",
            "decomposition_ok", "m_first", "m_second",
            "Q_recovered", "t_sum", "V_decomp", "verified"
        }
        assert set(result.keys()) == expected_keys
...
class TestEdgeCasesElectrolysis:
    ...
    def test_negative_cathode_overpotential_uses_abs(self):
        V = decomposition_voltage_jax(
            jnp.array(1.0e8), jnp.array(0.2e8),
            jnp.array(-0.3e8), jnp.array(0.05e8))
        expected = 1.0e8 + 0.2e8 + 0.3e8 + 0.05e8
        assert float(V) == pytest.approx(expected)

    def test_kohlrausch_zero_concentration(self):
        Lambda = kohlrausch_law_jax(
            jnp.array(149.9), jnp.array(0.0), jnp.array(94.0))
