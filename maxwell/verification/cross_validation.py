...
def validate_stress_energy_consistency() -> list[VerificationResult]:
    ...
    err = abs(T_trace - expected_trace) / abs(expected_trace) if expected_trace != 0 else abs(T_trace)
    results.append(VerificationResult(
        module_name="maxwell.electromagnetism",
        article_refs=(616, 637),
        test_name="Stress tensor trace = -(E^2+H^2)/(8pi)",
        expected=float(expected_trace), actual=float(T_trace),
        relative_error=float(err), tolerance=1e-10,
        passed=err <= 1e-10,
    ))
    ...
    err_p = abs(pressure - expected_pressure) / expected_pressure if expected_pressure != 0 else abs(pressure)
    results.append(VerificationResult(
        module_name="maxwell.electromagnetism.forces.stress_tensor",
        article_refs=(637, 640),
        test_name="EM pressure = (E^2+H^2)/(8pi)",
        expected=float(expected_pressure), actual=float(pressure),
        relative_error=float(err_p), tolerance=1e-10,
        passed=err_p <= 1e-10,
    ))
...
def validate_faraday_self_consistency() -> list[VerificationResult]:
    ...
    results.append(VerificationResult(
        ...
        expected=float(expected_emf), actual=float(emf),
        relative_error=float(err), tolerance=1e-8,
        passed=err <= 1e-8,
    ))
...
def validate_maxwell_equations_consistency() -> list[VerificationResult]:
    ...
    results.append(VerificationResult(
        ...
        expected=0.0, actual=float(gauss_e),
    ...
    results.append(VerificationResult(
        ...
        expected=0.0, actual=float(gauss_m),
...
def validate_cgs_si_roundtrip() -> list[VerificationResult]:
    ...
    results.append(VerificationResult(
        ...
        expected=q_esu, actual=float(q_back),
        relative_error=float(err_q), tolerance=1e-12,
        passed=err_q <= 1e-12,
    ))
    ...
    results.append(VerificationResult(
        ...
        expected=v_esu, actual=float(v_back),
        relative_error=float(err_v), tolerance=1e-12,
        passed=err_v <= 1e-12,
    ))
    ...
    results.append(VerificationResult(
        ...
        expected=B_gauss, actual=float(B_back),
        relative_error=float(err_B), tolerance=1e-12,
        passed=err_B <= 1e-12,
    ))
