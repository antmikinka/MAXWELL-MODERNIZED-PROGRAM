...
def _make(module: str, articles: tuple[int, ...], name: str,
          expected: float, actual: float, tol: float = 1e-8) -> VerificationResult:
...
def verify_spherical_harmonics() -> list[VerificationResult]:
    ...
    results.append(_make(
        "maxwell.math.spherical_harmonics", (128,), "P_0(0.5) = 1",
        expected=1.0, actual=calc_legendre_polynomial(0, 0.5),
    ))

    # P_1(x) = x
    results.append(_make(
        "maxwell.math.spherical_harmonics", (128,), "P_1(0.7) = 0.7",
        expected=0.7, actual=calc_legendre_polynomial(1, 0.7),
    ))
    ...
    results.append(_make(
        "maxwell.math.spherical_harmonics", (128,), f"P_2({x}) = {expected_p2}",
        expected=expected_p2, actual=calc_legendre_polynomial(2, x),
    ))
    ...
    results.append(_make(
        "maxwell.math.spherical_harmonics", (132, 685), "|Y_00| = 1/sqrt(4pi)",
        expected=expected_y00, actual=abs(y00),
    ))

    # Addition theorem for l=0: P_0(cos gamma) = 1
    add_l0 = addition_theorem(0, 0, 0, 0, 0)
    results.append(_make(
        "maxwell.math.spherical_harmonics", (143,), "Addition theorem l=0: P_0 = 1",
        expected=1.0, actual=add_l0, tol=1e-10,
    ))

    # Addition theorem for l=1, same direction: P_1(1) = 1
    add_l1_same = addition_theorem(1, np.pi/4, 0, np.pi/4, 0)
    results.append(_make(
        "maxwell.math.spherical_harmonics", (143,), "Addition theorem l=1, same dir: P_1(1) = 1",
        expected=1.0, actual=add_l1_same, tol=1e-10,
    ))
...
def verify_electrostatics() -> list[VerificationResult]:
    ...
    results.append(_make(
        "maxwell.core.charge", (27,), f"Point charge E at r={r}: q/r^2 = {expected_E}",
        expected=expected_E, actual=np.linalg.norm(E_at_r),
    ))
    ...
    expected_V = 1.0 / r
    results.append(_make(
        "maxwell.core.charge", (27,), f"Point charge V at r={r}: q/r = {expected_V}",
        expected=expected_V, actual=V_at_r,
    ))
    ...
    for th, ph in zip(theta, phi):
        pt = r_sphere * np.array([np.sin(th)*np.cos(ph), np.sin(th)*np.sin(ph), np.cos(th)])
        E = q.field_at(pt)
        n_hat = pt / r_sphere
    ...
    results.append(_make(
        "maxwell.core.charge", (41,), f"Gauss law flux: 4pi*q = {expected_flux:.4f}",
        expected=expected_flux, actual=flux, tol=0.01,
    ))
...
def verify_magnetism() -> list[VerificationResult]:
    ...
    results.append(_make(
        "maxwell.instruments.helmholtz", (475,),
        f"Helmholtz coil B_center > 0: {B_center:.6f}",
        expected=0.0, actual=B_center, tol=0.0,
    ))

    # Magnetic dipole: create a magnet and verify its moment
    north = MagneticPole(strength=1.0, position=np.array([0.0, 0.0, 1.0]), pole_type='N')
    south = MagneticPole(strength=1.0, position=np.array([0.0, 0.0, -1.0]), pole_type='S')
    ...
    results.append(_make(
        "maxwell.core.magnet", (371,),
        f"Magnetic moment = pole_strength * length = {expected_moment}",
        expected=expected_moment, actual=actual_moment,
    ))
...
def verify_electromagnetism() -> list[VerificationResult]:
    ...
    results.append(_make(
        "maxwell.electromagnetism.forces.lorentz", (593,),
        f"Lorentz Fy = -I*L*B = {expected_Fy:.6e}",
        expected=expected_Fy, actual=F[1], tol=1e-6,
    ))
    ...
    results.append(VerificationResult(
        ...
        expected=1.0, actual=float(is_symmetric),
    ...
    results.append(_make(
        "maxwell.electromagnetism.forces.stress_tensor", (616,),
        f"Stress tensor trace = -(E^2+H^2)/(8pi) = {expected_trace:.6e}",
        expected=expected_trace, actual=actual_trace,
    ))
...
def verify_vector_calculus() -> list[VerificationResult]:
    ...
    from maxwell.math.vector_operators import gradient, curl
    ...
    results.append(_make(
        "maxwell.math.vector_operators", (15,),
        f"grad(1/r) at (3,0,0): dV/dx = {expected_grad_x:.6e}",
        expected=expected_grad_x, actual=grad_val[0], tol=1e-4,
    ))
...
def verify_elliptic_integrals() -> list[VerificationResult]:
    ...
    K0 = ellipk(0.0)
    results.append(_make(
        "maxwell.math.elliptic_integrals", (696,), "K(0) = pi/2",
        expected=np.pi / 2, actual=K0,
    ))

    # E(0) = pi/2
    E0 = ellipe(0.0)
    results.append(_make(
        "maxwell.math.elliptic_integrals", (696,), "E(0) = pi/2",
        expected=np.pi / 2, actual=E0,
    ))
    ...
    results.append(_make(
        "maxwell.math.elliptic_integrals", (697,), "K(0.5) ~ 1.8540746773",
        expected=expected_K05, actual=K05, tol=1e-10,
    ))
    ...
    results.append(_make(
        "maxwell.math.elliptic_integrals", (697,), "E(0.5) ~ 1.3506438810",
        expected=expected_E05, actual=E05, tol=1e-10,
    ))
...
def verify_units_and_dimensions() -> list[VerificationResult]:
    ...
    results.append(_make(
        "maxwell.core.units", (771,), "ESU/EMU charge ratio / c = 1.0",
        expected=1.0, actual=ratio_over_c, tol=1e-10,
    ))

    # Speed of light constant
    results.append(_make(
        "maxwell.config.constants", (782,), "c = 2.9979e10 cm/s",
        expected=2.9979e10, actual=C, tol=1e-4,
    ))
...
def verify_optics_and_waves() -> list[VerificationResult]:
    ...
    results.append(_make(
        "maxwell.optics.wave_equation", (787,),
        f"Plane wave lambda = c/f = {expected_lambda:.4e} cm",
        expected=expected_lambda, actual=actual_lambda,
    ))
