...
class ElectrokineticEnergyJAX:
    ...
    def from_single_circuit(
        cls, inductance: float, current: float
    ) -> "ElectrokineticEnergyJAX":
    ...
    def from_coupled_circuits(
        cls, inductance_matrix: jax.Array, currents: jax.Array
    ) -> "ElectrokineticEnergyJAX":
    ...
    def energy_from_fields(
        self, A_potential: jax.Array, J_current: jax.Array, volume: float
    ) -> jax.Array:
        """Field formulation: T = (1/2) * A . J * V. Art. 634."""
        return self._fields_energy_jit(A_potential, J_current, volume)

    def energy_from_two_circuits(
        self, L1: float, L2: float, M: float, I1: float, I2: float
    ) -> jax.Array:
    ...
    @staticmethod
    def _coupled_energy_jit(
        inductance_matrix: jax.Array, currents: jax.Array
    ) -> jax.Array:
        """T = (1/2) * I^T . L . I."""
    ...
    @staticmethod
    def _fields_energy_jit(
        A_potential: jax.Array, J_current: jax.Array, volume: float
    ) -> jax.Array:
        """T = (1/2) * (A . J) * V."""
    ...
    @staticmethod
    def _two_circuit_energy_jit(
        L1: float, L2: float, M: float, I1: float, I2: float
    ) -> jax.Array:
...
class CoupledCircuitEnergyJAX:
    ...
    def from_currents_at(
        self, currents: jax.Array, inductance_matrix: jax.Array
    ) -> jax.Array:
...
@maxwell_cite(
    634,
    part=4,
    chapter="Electrokinetic Energy",
    description="Field formulation of electrokinetic energy",
)
def calc_electrokinetic_energy_jax(
    A_potential: jax.Array, J_current: jax.Array, volume: float
) -> jax.Array:
...
@maxwell_cite(
    635, part=4, chapter="Electrokinetic Energy", description="Single circuit energy"
)
...
@maxwell_cite(
    ...
    part=4,
    chapter="Electrokinetic Energy",
    description="Coupled circuits energy",
)
def calc_coupled_circuits_energy_jax(
    inductance_matrix: jax.Array, currents: jax.Array
) -> jax.Array:
    """Coupled circuits energy T = (1/2) * I^T . L . I. Arts. 636-637."""
...
@maxwell_cite(
    638, part=4, chapter="Electrokinetic Energy", description="Mutual inductance energy"
)
def calc_mutual_inductance_energy_jax(
    mutual_inductance: float, I1: float, I2: float
) -> jax.Array:
...
@maxwell_cite(
    ...
    part=4,
    chapter="Electrokinetic Energy",
    description="Two coupled circuit energy",
)
def calc_two_circuit_energy_jax(
    L1: float, L2: float, M: float, I1: float, I2: float
) -> jax.Array:
...
@maxwell_cite(
    638, part=4, chapter="Electrokinetic Energy", description="Coupling coefficient"
)
...
@maxwell_cite(
    ...
    part=4,
    chapter="Electrokinetic Energy",
    description="Verify coupled circuits energy consistency",
)
...
@maxwell_cite(
    ...
    part=4,
    chapter="Electrokinetic Energy",
    description="Comprehensive electrokinetic energy analysis",
)
def analyze_electrokinetic_energy_jax(
    ...
    if inductance_matrix is not None and currents is not None:
        result["matrix_energy"] = calc_coupled_circuits_energy_jax(
            inductance_matrix, currents
        )
        result["verification"] = verify_coupled_circuits_energy_jax(
            float(inductance_matrix[0, 0]),
            float(inductance_matrix[1, 1]),
            float(inductance_matrix[0, 1]),
            float(currents[0]),
            float(currents[1]),
        )["verified"]
